...
public class BugHuntPart36 : IDisposable
{
    ...
    // =====================================================================
    // Bug3626: Word section landscape readback should use friendly lengths
    // while preserving width > height semantics.
    // =====================================================================
    [Fact]
    public void Bug3626_Word_Section_Orientation_NoSwap()
    {
        ...
        var widthBefore = secBefore.Format.ContainsKey("pageWidth")
            ? WordSectionLengthAssertions.ParseCm(secBefore.Format["pageWidth"]) : 0m;
        var heightBefore = secBefore.Format.ContainsKey("pageHeight")
            ? WordSectionLengthAssertions.ParseCm(secBefore.Format["pageHeight"]) : 0m;
        ...
        var widthAfter = secAfter.Format.ContainsKey("pageWidth")
            ? WordSectionLengthAssertions.ParseCm(secAfter.Format["pageWidth"]) : widthBefore;
        var heightAfter = secAfter.Format.ContainsKey("pageHeight")
            ? WordSectionLengthAssertions.ParseCm(secAfter.Format["pageHeight"]) : heightBefore;

        widthAfter.Should().BeGreaterThan(heightAfter,
            "landscape readback should keep width greater than height after friendly formatting.");
    }
